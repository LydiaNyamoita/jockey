...
function Login() {
    ...
    onSubmit: async (values) => {
      try {
        const response = await fetch(
          "http://194.195.121.154:5500/api/auth/login",
          {
            method: "POST",
            headers: {
        ...
        if (response.status === 200) {
          ...
          if (userData && userData.data && userData.data.token) {
            ...
            const authResponse = await fetch(
              "http://194.195.121.154:5500/ping/auth",
              {
                method: "GET",
                headers: {
  ...
  return (
    <div className="flex items-center  justify-center my-14 mt-[150px] bg-white ">
      <div className="hidden lg:block bg-[#E6F7F5] ">
        <img src={horse} alt="horse" className="w-[550px] h-[700px] " />
      </div>
